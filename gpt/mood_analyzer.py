...
async def analyze_mood(prompt: str) -> dict:
    try:
        ...
        system_msg = (
            ...
            "could be used to search for matching songs "
            "(e.g., 'uplifting indie pop', 'calm piano', 'aggressive rock')\n"
            "The query should be based on the mood and suitable for use in"
            "YouTube Music or Spotify search.\n"
            "Try to make the query resemble something a human would search for"
            "on Spotify or YouTube Music. "
            "Prefer concise, natural language — no generic or stock-sounding phrases"
            "like 'Energetic Upbeat Pop'. "
            "Avoid instrumental ambient or meditation music unless the user's mood"
            "explicitly suggests they want that. "
            ...
            "The goal is to find short, high-quality, and popular tracks"
            "from real artists, not background or stock music.\n"
            "Respond with valid JSON **only**. No preamble, explanation,"
            "or formatting — just raw JSON."
        )
